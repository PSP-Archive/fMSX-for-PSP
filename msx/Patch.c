...
{
#ifdef DISK
  if(Verbose&0x04) _printf("CHECK DISK %c\n",R->AF.B.h+'A');
...
****************************************************************/
{
  ...
  P=BootBlock+3;
  N=2-R->AF.B.h;
  _memcpy(P,"fMSXdisk",8);P+=10;    /* Manufacturer's ID   */
  ...
  *P++=Info[N].Heads;*P=0x00;      /* Number of heads     */
  ...
  for(_memset(Buf,0x00,512);J;J--)
    if(!DiskWrite(R->DE.B.h,Buf,Sector++)) { R->AF.W=0x0A01;return; }
  for(_memset(Buf,0xFF,512);I;I--)
    if(!DiskWrite(R->DE.B.h,Buf,Sector++)) { R->AF.W=0x0A01;return; }
...
/*
{
  ...
  if(Verbose&0x04) _printf("TAPE: Looking for header...");
...
default:
  _printf("Unknown BIOS trap called at PC=%04Xh\n",R->PC.W-2);
