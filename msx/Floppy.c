...
/*************************************************************/
char	Dsk_buffer[1][DSK_DISK_SIZE];
int		Dsk_buffer_c=0;
...
*/
	if(!Dsk){
		if( Dsk_buffer_c>=1 ){
			Error_mes("ディスク3枚超えてるしよ");
			return 0;
		}
		Dsk=(byte *)&Dsk_buffer[Dsk_buffer_c][0];//malloc(DSK_DISK_SIZE);
		Dsk_buffer_c++;
	}
  ...
  /* Clear disk image */
  _memset(Dsk,0x00,DSK_DISK_SIZE);

  /* Boot sector parameters */
  Dsk[0x00] = 0xC9;
  Dsk[0x01] = 0xC9;
  Dsk[0x02] = 0xC9;
  _memcpy(Dsk+3,"MSX-DISK",8);
...
int DSKFile(byte *Dsk,const char *FileName)
{
  ...
  for(J=1;J<=DSK_DIR_SIZE;J++)
    if(DSKFileName(Dsk,J)&&!_memcmp(FileName,DSKFileName(Dsk,J),11))
      return(0);
...
const char *DSKFileName(const byte *Dsk,int ID)
{
  ...
  return(!Name[0]||(Name[0]==0xE5)? 0:(const char *)Name);
}
...
int DSKWrite(byte *Dsk,int ID,const byte *Buf,int Size)
{
  ...
  for(Written=0;(J!=0xFFF)&&(Written<Size);J=I)
  {
    ...
    _memcpy(P+DSK_CLUSTER_SIZE*(J-2),Buf,I);
    Buf    += I;
    Written+= I;
...
int DSKRead(const byte *Dsk,int ID,byte *Buf,int Size)
{
  ...
  for(Read=0;(Read<Size)&&(J>1)&&(J<0xFF1);)
  {
    ...
    _memcpy(Buf,P+DSK_CLUSTER_SIZE*(J-2),I);
    Buf += I;
    Read+= I;
    P    = Dsk
         + DSK_SECTOR_SIZE*(DSK_RESERVED_SECS+DSK_SECS_PER_BOOT)
